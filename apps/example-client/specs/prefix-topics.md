...
- As a dashboard developer, I want to subscribe to `/MyTable/Accelerometer/` once and receive values for `X`, `Y`, and `Z` sub-topics as they appear.
...
| ID    | Description                                                                                                                                                                |
| ----- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| PFX-1 | `createPrefixTopic(prefix)` + `subscribe(cb)` shall deliver values for every topic whose name starts with `prefix`.                                                        |
...
| PFX-3 | A scalar topic (e.g. `/MyTable/AutoMode`) published by the client shall be readable via a fresh `createTopic(...).subscribe(...)` while the prefix subscription is active. |

## Tests

| Test                                                                                      | Covers       | Status      |
| ----------------------------------------------------------------------------------------- | ------------ | ----------- |
| `[PFX-1, PFX-2] receives /MyTable/Accelerometer/{X,Y,Z} via a single prefix subscription` | PFX-1, PFX-2 | Implemented |
